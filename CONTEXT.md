...
branch `main`, HEAD `7cf8c73`, `origin/main`과 같습니다.
문서 3개(두 feature README와 implement.md)에 아래 결정을 기록한 uncommitted 변경이 남아 있습니다.
...
- **`SPEC §5.3` 재판정의 방향은 「성립」이고, 판정 자체는 task-012 approve 시점에 확정합니다.**
  ...
  같은 README 2026-08-29 마지막 항목과 implement.md task-012 `확인` 필드에 있습니다.
...
- **UI 테스트 중복 정리는 기기 인증 창이 풀리고 task-011·012가 UI 스위트를 요구하는 시점에 함께 합니다.**
...
- **테스트 통과 건수는 baseline으로 쓰지 않습니다.** 기록과 verify 대조의 기준은 「실패 0 + `** TEST SUCCEEDED **`」이고,
  건수는 필요할 때 `Test case … passed` 줄 수를 보조로만 덧붙입니다.
  같은 README 2026-08-29 마지막 항목에 갈렸던 네 값과 함께 있습니다.
...
- 색 팔레트를 확정했습니다. CPU User `#2a78d6`/`#3987e5`, System `#eb6834`/`#d95926`,
  ...
  근거는 rv README task-004 항목에 있습니다.
...
- [features/20260818-001-resource-visualization/README.md](./features/20260818-001-resource-visualization/README.md) (변경함)
  — 2026-08-29 항목 둘에 task-008 결과·`MemoryDetailView` 뷰 본문 그물 0 한계와 위 결정 셋의 근거
...
- [ResourceRunnerTests/MemoryCompositionDetailTests.swift](./ResourceRunnerTests/MemoryCompositionDetailTests.swift)
