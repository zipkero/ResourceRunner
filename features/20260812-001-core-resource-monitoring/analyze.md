...
- 자식 팝오버는 key window를 가져가지 않습니다.
  상세 팝업을 연 직후, 팝업 안 정적 텍스트를 클릭한 뒤, 팝업 안 disclosure 컨트롤을 클릭한 뒤,
  그 뒤 Space를 입력한 뒤까지 다섯 시점에서 접근성 계층을 덤프했고
  `Keyboard Focused`는 매번 부모 팝오버에 있었습니다.
  그래서 부모 뷰 계층에 등록한 단축키가 팝업이 열린 뒤에도 계속 닿습니다(§5 DP15).
  팝업 콘텐츠 쪽에 같은 단축키를 등록해도 그 등록은 키 이벤트를 받지 않으며,
  등록의 동작을 반대로 바꿔 놓아도 어떤 관측에서도 차이가 나타나지 않는 것으로 확인했습니다.
...
- 채택안: 옵션 A.
  ...
  등록 자리는 대시보드 본체 한 곳입니다.
  이 판단은 처음에 팝업 콘텐츠 쪽 등록을 함께 두는 것으로 정했다가 실측 후 바꿨습니다 —
  상세 팝업이 열린 뒤 키 이벤트가 어느 창으로 가는지 확인되지 않아 양쪽에 등록해 두려 했으나,
  자식 팝오버가 key window를 가져가지 않는 것이 확인됐습니다(§근거 확인 사실).
  팝업 쪽 등록은 키 이벤트를 받지 않으므로 존재하든 없든, 심지어 동작이 틀리게 바뀌어도
  어떤 관측으로도 구분할 수 없는 코드가 됩니다. 검증할 수 없는 중복은 회귀를 잡아 주지 않으면서
  안전하다는 오해만 만들므로 두지 않습니다.
  대가는 macOS나 SwiftUI가 팝오버의 key window 처리를 바꾸면 그 회귀를 잡을 그물이 없다는 것입니다.
  단축키의 존재는 카드에 항상 보이는 표시와 카드 접근성 이름에서 확인할 수 있어야 하며,
  표시 문자열은 단축키 정의와 같은 상수에서 유도해 키를 바꿀 때 표시가 뒤처지지 않게 합니다.
