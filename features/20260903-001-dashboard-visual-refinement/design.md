...
**이 결정은 이후 대체됐습니다.**
`20260912-001-dashboard-visual-language`가 카드 면을 바탕보다 띄우는 쪽으로 다시 정해
(그 feature `spec.md`의 「라이트 모드 카드 면은 L3안」과 완료 조건 「카드가 바탕보다 떠올라 보입니다」),
현재 코드의 카드 표면은 옵션 C의 `strokeBorder`가 아니라
`RoundedRectangle(cornerRadius:).fill(DashboardStyle.CardSurface.fillColor)`입니다.
위 옵션 비교와 채택 근거는 2026-09-04 시점의 결정 기록으로 남깁니다.
`contentShape`를 함께 거는 결정은 그대로 유효합니다.
...
**이 결정은 이후 대체됐습니다.**
`20260906-001-graph-legibility-and-color`가 카드 확대를 허용하는 쪽으로 `SPEC §5.10`을 되열어,
현재 `DashboardCardLayoutTests`에는 상한 상수 둘(`previousCPUUpperBound`·`previousMemoryUpperBound`)이 없고
`previousCPUHeight`로 「변경 전보다 작아지지 않는다」를 단언합니다.
위 요구는 2026-09-04 시점의 결정 기록으로 남깁니다.
...
**카드 표면 항목은 이후 대체됐습니다.**
`20260912-001-dashboard-visual-language`가 카드 면을 다시 불투명 채움으로 정해(§5 DP3의 대체 기록),
위 「카드 테두리 한 경로」와 「카드 배경이 채움에서 선으로 바뀝니다」는 현재 코드에 해당하지 않습니다.
현재 카드 표면은 이 feature 이전과 같은 `RoundedRectangle().fill()` 구조라 이 항목의 증가분이 0입니다.
